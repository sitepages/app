...
import { useInstallments, fetchInstallmentPayments } from '@/hooks/useInstallments'
...
export default function InstallmentsPage() {
  ...
  async function handleExpandPayments(planId: string) {
    if (expandedId === planId) { setExpandedId(null); return }
    setLoadingPay(true)
    const data = await fetchInstallmentPayments(planId)
    setPayments(data)
...
function InstallmentCard({ plan, onUpdateCurrent, onClose, onDelete,
  ...
  return (
    <div className="card px-5 py-4"
      ...
      <div className="flex items-start gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {plan.needs_review && (
              <span className="badge badge-warning" style={{ fontSize: 10 }}>⚠ Revisar</span>
            )}
          </div>
      ...
      {/* Histórico */}
      <button onClick={onExpandPayments}
        style={{ background: 'none', border: 'none', cursor: 'pointer',
                 fontSize: 12, color: 'var(--text-muted)', padding: '8px 0 0 0',
                 display: 'flex', alignItems: 'center', gap: 4 }}>
        {isExpanded ? '▲' : '▼'} {isExpanded ? 'Ocultar' : 'Ver'} histórico de pagamentos
      </button>
        ...
        <div style={{ marginTop: 8, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
          {loadingPayments ? (
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Nenhum pagamento registrado ainda. Importe o próximo extrato para registrar automaticamente.
            </p>
          ) : (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {Array.from({ length: plan.total_installments }, (_, i) => {
                const num = i + 1
                const pay = payments.find((p: any) => p.installment_number === num)
                return (
                  <div key={num} style={{
                    padding: '6px 10px', borderRadius: 8, fontSize: 11,
                    background: pay ? 'var(--success-bg)' : 'var(--bg-elevated)',
                    border: `1px solid ${pay ? 'rgba(0,196,122,0.2)' : 'var(--border)'}`,
                    color: pay ? 'var(--success)' : 'var(--text-muted)',
                    minWidth: 70, textAlign: 'center',
                  }}>
                    <p style={{ fontWeight: 700 }}>#{num}</p>
                    {pay ? (
                      <>
