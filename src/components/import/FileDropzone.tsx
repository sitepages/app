...
export default function FileDropzone({ onFile, disabled }: FileDropzoneProps) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`relative border-2 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center gap-4 text-center transition-all duration-200 cursor-pointer ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${dragging ? 'border-indigo-500 bg-indigo-500/10' : selected ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-slate-700 bg-slate-900 hover:border-slate-600 hover:bg-slate-800/50'}`}
    >
      <input
        ref={inputRef}
        type="file"
